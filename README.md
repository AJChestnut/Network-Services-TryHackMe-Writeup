...
This writeup will include the answers to most questions, but, as requested by TryHackMe, it will not include the Flags or password. 
...
This tasks teaches you about the Microsoft service Server Message Block (SMB), which is a client-server protocol used for sharing access to files, printers, and other resources across a network. As it is a theory task, read through the information and use it to answer the below questions. If you've never seen the word NetBEUI before, and I wouldn't blame you if you hadn't, it is prounounced Net Buoy, like an ocean buoy.  

If you'd like to minimize your wait time, I would recommend opening up Task 3 and clicking the green "Start Machine" button now, then scroll back up and run through this task.
...
Let's get started with enum4linux, condust a full basic enumeration. For starters, what is the **workgroup** mame?
...
For this question, you are going to need to scroll down to the "Host Script Results" section of the scan results if you used `nmap -A -p-`.  
...
IPC$ is Interprocess communication, not going to need that. print$ is for printer drivers, also not really what we want. netlogon for the Network Logon Service, probably not going to have the information we need. That leaves `profiles` which contains information about User Profiles. That has potential. We may be able to find a user's logon information there. 

## Task 4 Exploiting SMB

This is where we get into even more fun. We are going to be using the SMBClient which is available on Kali but can be added to other distros. For the smbclient we are going to be using the syntax `smbclient //{IP address}/{Share Name} -U {Username} -p {Port Number}`.

**Question 1**

What would be the correct syntax to access an SMB share called "secret" as user "suit" on a machine with the IP 10.10.10.2 on the default port?

Looking at the syntax above, we can simply plug in the informatioin.
...
Lets see if our interesting share has been configured to allow anonymous access, I.E(sic) it doesn't require authentication to view the files. We can do this easily by:
...
The first one to jump out at me was the text file. Now keep in mind, Linux does not like files with spaces in their names, so you are going to have to add " " around it in your commands. Speaking of, as the terminal says, typing "help" will get you a list of the available commands; which I found very useful. 

Normally, I would use `cat` to display the contents of the .txt file, but it's not in the list. It seems, smbclient doesn't know that one, so I went with `more`.

![T4 Q5](https://user-images.githubusercontent.com/84687845/125152442-d4700b00-e109-11eb-8725-3fe758830f92.JPG)

This file gives us two big pieces of information, a potential user name, based on John Cactus, the addresse of the file, and that John has an SSH account set up.
...
I think `get` would work a treat. So, to download the file, while still in the .ssh folder, you are going to run `get id_rsa`.
On the Kali browser-based machine, that'll drop the file into your root directroy. If you are using a locally run Linux distro, it'll probably drop it into your user's home directory. 
...
As this completes the SMB section, you should terminate the current Active Machine, then jump down to Task 6, and start that machine, before returning to Task 5.

## Task 5 Understanding Telnet

Telnet is an outdated application protocol. It is used to interact with remote hosts. Once connected to the remote system, it becomes a virtual terminal for that system. However, it's outdated because it sends all of its information in clear text; there is no encryption. Since it is much more secure, SSH has supplanted telnet in most situations. To connect to a remote system using telent, you use the syntax `telnet {IP Address} {Port Number}`. The default port for telnet is 23.
...
Just like with the SMB tasks, we are going to start out by running a port scan of our target system.

Run `nmap -A -p- {IP address of your Active Machine}` This scan may take a while. Mine completed in 174.56 seconds.
...
Keeping notes are very important. Writing reports of our findings is a significant of being in security. Gettig in the habit of thorough note taking makes everything easier.
...
For this you are going to need to run, `telnet {Your Active Machine's IP Address} 8012` becasue even though the default port for telnet is 23, as we found in our nmap search, this system has it on a non-standard port.
...
Seeing as all of your commands are being sent into the void, it's a little difficult to close this connection. To get your prompt to change to something helpful, press `ctrl + ]`. That is control and the right square bracket key at the same time, which should change your prompt to `telnet>`, then type `close` and hit enter. You should now be back at your usual prompt. Or, if you'd like you can leave the telnet session connected and open a new tab/window to run the following tcpdump.

Start a tcpdump listener on your local machine.

A tcpdump listener is going to monitor the selected interface for traffic and then log that traffic on the screen. You are going to start it and then leave it running in its own tab/window.
...
For this question we are going to be using the telnet session again, if you still have it open, great. If not, open a new tab/window and reconnect to the machine using the same steps from above. Make note, you are going to need the IP address of the system you are working on, either the Attack Box, Kali Machine, or your local system for this question.

This question is having us run the ping command through the telnet session. We're going to enter `.RUN ping {Local IP Address} -c 1`. That is going to send 1 ping from the remote host to our local machine. If you can have both of the windows in view at the same time, you can see what happens as it happens.
...
From there you can enter commands like normal and send them to the remote system.
...
We are going to be using nmap again to enumerate this FTP server. Looking through the next questions, you are going to need more information than you'd receive from a basic scan, so I went with `nmap {Active Machine IP Address} -A -p-`. 
...
For this question, you are going to use the `get` command again, just like with telnet. Enter `get PUBLIC_NOTICE.txt`

That'll download the file to your system so that you can view its contents. You don't need to be connected to the ftp server anymore so enter `exit` to return to your normal prompt. 
...
Once you've found it on your system, it's time to read it. Either navigate to the folder it's in or enter the whole file path using the `cat` command. `cat PUBLIC_NOTICE.txt` and see if you can see any information that could be usful to us, something that may be a username perhaps?
...
For this particular task, we are going to be using the username `mike` that we already know, and attempt to brute force his password. This is where the `rockyou.txt` file comes into play, it is a password list. If you are using your own local system, remember where you downloaded it to and use that for the file path. The tool we are going to be using is called hydra. Hydra can take in a password list and attempt them against a system to see if any of them work.

The tryhackme page has a great breakdown of the systax we are going to be using for this attack. I'd highly suggest reading through it.

`hydra -t 4 -l dale -P /usr/share/wordlists/rockyou.txt -vV 10.10.10.6 ftp`

Depending on what system you are using, your file path may vary. Also, if you are on the browser-based Kali machine, you are going to have an extra step. On the Kali machine the wordlist is compressed into a GZ file. We need to unzip it. In your terminal enter `gzip -d /root/Desktop/wordlists/rockyou.txt.gz`. After a moment, you'll have the uncompressed file you need.
...
![T10 Q1](https://user-images.githubusercontent.com/84687845/125152618-e7370f80-e10a-11eb-9f45-50eee662f2e3.JPG)
...
Use `ls` to see the contents of the diretory. Two files here. The .txt one is probably the one that's gonna be human readable. Use the `get` command to download it to your system. Now that you've done this a couple times, you should be able to find the file and view its contents. If you're struggling, you can scroll up and find the instructions from the other tasks. 
